...
namespace Wang.MTHDAL
{
    ...
    {
        //读取App.config配置文件获得连接字符串
        private readonly static string connString = ConfigurationManager.ConnectionStrings["connString"].ToString();
        ...
        public static DataSet GetDataSet(string sql, SqlParameter[] paramArray = null, string tableName = null)
        {
            ...
            {
                throw new Exception("执行public DataSet GetDataSet(string sql, string tableName = null)方法发生异常：" + ex.Message);
            }
            finally
            {
                conn.Close();
